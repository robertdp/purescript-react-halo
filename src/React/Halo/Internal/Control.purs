...
data HaloF props state action (m :: Type -> Type) a
  = Props (props -> a)
...
-- | - `m` is the monad used during evaluation
-- | - `a` is the result type
newtype HaloM props state action (m :: Type -> Type) a = HaloM (Free (HaloF props state action m) a)
...
-- | - `m` is the monad used during evaluation
-- | - `a` is the result type
newtype HaloAp props state action (m :: Type -> Type) a = HaloAp (FreeAp (HaloM props state action m) a)
